...
class MachineRepository {
    ...
    public updateRequests = async(id: string): Promise<IQueryResponse> => {
    ...
    public editProduct = async(id: string): Promise<IQueryResponse> => {

        return database.query(
            `UPDATE products SET quantity = quantity - 1 WHERE id = '${id}' RETURNING *`
        ).then(async(value) => {
