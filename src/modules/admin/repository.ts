...
class AdminsRepository {
    ...
    public signup = async (user: IUser): Promise<IQueryResponse> => {
        ...
        return database.query(
            `insert into ${this.table}(first_name, last_name, email, password, role) values('${user.first_name}', '${user.last_name}', '${user.email}', '${user.password}', 2) RETURNING *`
        )
            .then((value) => {
                return {
                    ok: true,
    ...
    public getAll = async (): Promise<IQueryResponse> => {
        ...
        return database.query(`SELECT * FROM ${this.table} WHERE role = 2`)
            .then((value) => {
                return {
                    ok: true,
                    data: value.rows
    ...
    public update = async (id: string, user: IUser): Promise<IQueryResponse> => {
        ...
        return database.query(`UPDATE ${this.table} SET(first_name, last_name, email) = ('${user.first_name}', '${user.last_name}', '${user.email}') WHERE id = '${id}' AND role = 2 RETURNING *`)
    ...
    public delete = async (id: string): Promise<IQueryResponse> => {
        ...
        return database.query(`delete from ${this.table} WHERE id = '${id}' AND role = 2`)
            .then((value) => {
                if (value.rowCount === 0) return {
                    ok: false,
                    data: 'User not found'
